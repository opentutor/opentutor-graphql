...
export const updateSession = {
  ...
  args: {
    ...
    dialogApiKey: {type: GraphQLString}
  },
  resolve: async (
    _root: GraphQLObjectType,
    args: { sessionId: string; session: string, dialogApiKey: string },
  ): Promise<Session> => {
    if(!args.dialogApiKey){
      throw new Error("missing required dialogApiKey")
    }
    const dialogApiKey = process.env.DIALOG_API_KEY;
    if(dialogApiKey !== args.dialogApiKey){
      throw new Error("invalid api key recieved")
    }
    if (!args.sessionId) {
